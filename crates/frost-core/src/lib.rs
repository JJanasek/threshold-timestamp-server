...
// This tells Rust to look for secp256k1.rs
pub mod secp256k1;
...
impl fmt::Display for CryptoError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidThreshold { k, n } => write!(f, "invalid threshold: k={k}, n={n}"),
            Self::InvalidIndices => write!(f, "participant indices must be distinct"),
            Self::InsufficientShares { got, need } => write!(f, "insufficient shares: need {need}, got {got}"),
            Self::InvalidEncoding => write!(f, "could not decode scalar/point"),
            Self::VerificationFailed => write!(f, "signature verification failed"),
            Self::Frost(e) => write!(f, "FROST error: {e}"),
        }
    }
}
